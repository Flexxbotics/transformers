...
Example client for the PL Press report server.

Two ways to call it are shown:

1. ``PLPressReportClient`` - a dependency-free client using ``requests``
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Query the PL Press report server")
    ap.add_argument("--url", default="http://localhost:8757", help="Base URL of the report server")
    ...
    c = PLPressReportClient(args.url, token=args.token)
